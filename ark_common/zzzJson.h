...
typedef BOOL zzz_BOOL;
static const zzz_BOOL zzz_True = TRUE;
static const zzz_BOOL zzz_False = FALSE;
...
static inline void zzz_Free(void* pointer) { _free(pointer); }
...
static inline void* zzz_New(zzz_SIZE size) { return AllocMemorySize += size, AllocMemoryCount += 1, pure_malloc(size); }
static inline void zzz_Free(void* ptr) { FreeMemoryCount += 1, _free(ptr); }
...
static zzz_BOOL zzz_ValueParseFast(struct zzz_Value* v, const char* s);
...
static const char* zzz_ValueStringify(const struct zzz_Value* v);
...
static const zzz_JSONType* zzz_ValueType(const struct zzz_Value* v);
...
static inline zzz_BOOL zzz_ValueSetNull(struct zzz_Value* v);

static inline zzz_BOOL zzz_ValueSetBool(struct zzz_Value* v, zzz_BOOL b);
...
static inline zzz_BOOL zzz_ValueSetArray(struct zzz_Value* v);

static inline zzz_BOOL zzz_ValueSetObj(struct zzz_Value* v);
...
static zzz_BOOL zzz_ValueObjDel(struct zzz_Value* v, const char* key);
...
static inline const double* GetDouble(struct zzz_Value* v)
{
	return zzz_ValueGetDouble(v);
}
static inline const int* GetInt(struct zzz_Value* v)
{
	return zzz_ValueGetInt(v);
}
static inline const long* GetLong(struct zzz_Value* v)
{
	return zzz_ValueGetLong(v);
}
static inline const long long* GetLongLong(struct zzz_Value* v)
{
	return zzz_ValueGetLongLong(v);
}
...
static inline BOOL SetNull(Value* v)
{
	return zzz_ValueSetNull(v);
}
static inline BOOL SetBool(Value* v, BOOL b)
{
	return zzz_ValueSetBool(v, b);
}
...
static inline BOOL SetArray(Value* v)
{
	return zzz_ValueSetArray(v);
}
static inline BOOL SetObj(Value* v)
{
	return zzz_ValueSetObj(v);
}
...
{
	return _atoi((const unsigned char*)str);
}
...
static inline zzz_BOOL zzz_AllocatorAppendChild(zzz_SIZE init_size, struct zzz_Allocator* alloc)
{
	// 每次分配一大块内存，避免多次分配
...
static inline char* zzz_AllocatorAlloc(struct zzz_Allocator* alloc, zzz_SIZE size)
{
	...
	if (zzz_UNLIKELY(cur_node->Pos + size > s))
	{
		...
		if (zzz_UNLIKELY(zzz_AllocatorAppendChild(s, alloc) == zzz_False))
			return 0;
		cur_node = alloc->End;
	}
...
// 追加字符串
static inline zzz_BOOL zzz_StringAppendStr(struct zzz_String* str, const char* s, zzz_SIZE size)
{
...
// 追加字符
static inline zzz_BOOL zzz_StringAppendChar(struct zzz_String* str, const char c)
{
	zzz_SIZE src_s = str->Size;
...
// 追加结束符
static inline zzz_BOOL zzz_StringAppendEnd(struct zzz_String* str)
{
	return zzz_StringAppendChar(str, 0);
}
...
static void zzz_UnEscapeStr(const char* str, zzz_SIZE len, char* s)
{
	zzz_SIZE s_index = 0;
	zzz_SIZE index;
	char c;
...
// 消费字符串
static zzz_BOOL zzz_ConsumeStr(const char* s, zzz_SIZE* index)
{
	char c;
	c = s[(*index)++];
...
static zzz_BOOL zzz_CheckStr(const char* s, zzz_SIZE* len)
{
	zzz_SIZE index = 0;
	char c;
	c = s[index++];
...
static inline zzz_BOOL zzz_CheckStrLen(struct zzz_Allocator* alloc, const char* s, zzz_SIZE len)
{
	...
	if (zzz_UNLIKELY(zzz_StringAppendStr(zzz_StringCache, s, len) == zzz_False))
		return zzz_False;
...
static inline zzz_BOOL zzz_CheckNumLen(struct zzz_Allocator* alloc, const char* s, zzz_SIZE len)
{
	...
	if (zzz_UNLIKELY(zzz_StringAppendStr(zzz_StringCache, s, len) == zzz_False))
		return zzz_False;
	if (zzz_UNLIKELY(zzz_StringAppendEnd(zzz_StringCache) == zzz_False))
		return zzz_False;
	zzz_SIZE avail_len;
...
static inline zzz_BOOL zzz_ValueParseLen(struct zzz_Value* v, const char* s, zzz_SIZE len)
{
	...
	if (zzz_UNLIKELY(str == 0)) return zzz_False;
	zzz_Copy(s, len, str);
	str[len] = 0;
	return zzz_ValueParseFast(v, s);
}
...
static const char* zzz_ValueStringify(const struct zzz_Value* v)
{
	...
	case zzz_JSONTYPEARRAY:
		if (zzz_UNLIKELY(zzz_StringAppendChar(str, '[') == zzz_False))
			return 0;
	...
	case zzz_JSONTYPESTRING:
		if (zzz_UNLIKELY(zzz_StringAppendChar(str, '"') == zzz_False))
			return 0;
		if (zzz_UNLIKELY(zzz_StringAppendStr(str, node->Value.Str, node->Len) == zzz_False))
			return 0;
		if (zzz_UNLIKELY(zzz_StringAppendChar(str, '"') == zzz_False))
			return 0;
		break;
	...
	while (zzz_LIKELY(node != v->N))
	{
		if (node->Key != 0)
		{
			if (zzz_UNLIKELY(zzz_StringAppendChar(str, '"') == zzz_False))
				return 0;
			if (zzz_UNLIKELY(zzz_StringAppendStr(str, node->Key, node->KeyLen) == zzz_False))
				return 0;
			if (zzz_UNLIKELY(zzz_StringAppendStr(str, "\":", 2) == zzz_False))
				return 0;
		}
		switch (node->Type)
		{
	...
	if (zzz_UNLIKELY(zzz_StringAppendEnd(str) == zzz_False))
		return 0;
...
static inline const char* zzz_ValueGetStr(struct zzz_Value* v)
{
	...
	if (zzz_UNLIKELY(str == 0)) return 0;
...
static inline const char* zzz_ValueGetUnEscapeStr(struct zzz_Value* v)
{
	...
	if (zzz_UNLIKELY(ret_str == 0)) return 0;
...
static zzz_BOOL zzz_ValueCopyFrom(struct zzz_Value* v, const struct zzz_Value* vv)
{
	...
	do
	{
		...
		if (node->Key != 0)
		{
			...
			if (zzz_UNLIKELY(k == 0)) return zzz_False;
		...
		case zzz_JSONTYPEARRAY:
			...
			if (zzz_LIKELY(node->Value.Node != 0))
			{
				...
				if (zzz_UNLIKELY(n == 0)) return zzz_False;
				n->Father = des_node;
				n->Prev = 0;
		...
		case zzz_JSONTYPESTRING:
		{
			...
			if (zzz_UNLIKELY(s == 0)) return zzz_False;
...
static inline struct zzz_Value* zzz_ValueCopy(const struct zzz_Value* v)
{
	...
	if (zzz_UNLIKELY(zzz_ValueCopyFrom(ret_val, v) == zzz_False))
		return 0;
	return ret_val;
}
...
static inline zzz_BOOL zzz_ValueSetDouble(struct zzz_Value* v, const double d)
{
	...
	if (zzz_UNLIKELY(num == 0)) return zzz_False;
